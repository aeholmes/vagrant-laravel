class mysql 
{
    ...
    {
      "create-vagrant-db":
      unless => "/usr/bin/mysql -uroot -p$mysqlPassword vagrant",
      command => "/usr/bin/mysql -uroot -p$mysqlPassword -e 'create database `vagrant`;'",
      require => [Service["mysql"], Exec["set-mysql-password"]]
    }
    ...
    {
      "grant-vagrant-db":
      command => "/usr/bin/mysql -uroot -p$mysqlPassword -e 'grant all on `vagrant`.* to `root@localhost`;'",
      require => [Service["mysql"], Exec["create-vagrant-db"]]
    }
}
